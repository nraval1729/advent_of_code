...
func main() {
	...
	ansPartOne, err := computePartOne(expenseReport, targetSum)
	if err != nil {
		panic(err)
	}
	fmt.Printf("Part one answer: %d\n", ansPartOne)

	ansPartTwo, err := computePartTwo(expenseReport, targetSum)
	if err != nil {
		panic(err)
	}
...
func computePartOne(expenseReport []int, targetSum int) (int, error) {
	expensesMap := make(map[int]bool)

	for _, expense := range expenseReport {
		expensesMap[expense] = true
	}

	for _, expense := range expenseReport {
		if _, ok := expensesMap[targetSum-expense]; ok {
			return expense * (targetSum - expense), nil
		}
	}
	return -1, fmt.Errorf("No expense pair summing up to %d found in partOne!\n", targetSum)
}

func computePartTwo(expenseReport []int, targetSum int) (int, error) {
	expensesMap := make(map[int]bool)
	...
	for _, firstExpense := range expenseReport {
		for _, secondExpense := range expenseReport {
			if _, ok := expensesMap[targetSum-(firstExpense+secondExpense)]; ok {
				return firstExpense * secondExpense * (targetSum - (firstExpense + secondExpense)), nil
			}
		}
	}
	return -1, fmt.Errorf("No expense triplet summing up to %d found in partOne!\n", targetSum)
}
