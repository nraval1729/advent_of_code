...
func main() {
	...
	fmt.Printf("Solution for part 1: %d\n", computePartOne(course))
	fmt.Printf("Solution for part 2: %d\n", computePartTwo(course))

}

func computePartOne(course []string) int {
	var horizontalPos, verticalPos int

	for _, command := range course {
		commandSplit := strings.Split(command, " ")
		action := commandSplit[0]
		value, _ := strconv.Atoi(commandSplit[1])
...
func computePartTwo(course []string) int {
